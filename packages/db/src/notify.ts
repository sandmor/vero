...
function getNotifyPool(): Pool {
  if (!notifyPool) {
    ...
    if (!connectionString) {
      throw new Error(
        'DATABASE_URL environment variable is required for notifications'
      );
    }
    notifyPool = new Pool({
      connectionString,
...
export async function notifyChatCreated(
  ...
  await notifyChatChange(
    { userId, chatId, action: ChatAction.CREATED },
    client
  );
}
...
export async function notifyChatUpdated(
  ...
  await notifyChatChange(
    { userId, chatId, action: ChatAction.UPDATED },
    client
  );
}
...
export async function notifyChatDeleted(
  ...
  await notifyChatChange(
    { userId, chatId, action: ChatAction.DELETED },
    client
  );
}
