...
export function TimeSeriesWidget({
  ...
  const yAxisFormatter =
    metric === 'cost' ? (v: number) => `$${v}` : formatNumber;
...
export function TokenBreakdownWidget({
  widget,
  totalTokens,
}: TokenBreakdownWidgetProps) {
  const pieData = useMemo(
    () =>
      [
        {
          name: 'Input',
          value: totalTokens.inputTokens,
          color: TOKEN_TYPE_COLORS.input,
        },
        {
          name: 'Output',
          value: totalTokens.outputTokens,
          color: TOKEN_TYPE_COLORS.output,
        },
        {
          name: 'Reasoning',
          value: totalTokens.reasoningTokens,
          color: TOKEN_TYPE_COLORS.reasoning,
        },
        {
          name: 'Cached',
          value: totalTokens.cachedInputTokens,
          color: TOKEN_TYPE_COLORS.cached,
        },
      ].filter((t) => t.value > 0),
    [totalTokens]
  );
...
export function ModelDistributionWidget({
  ...
  const { chartData, otherModels, hasOthers } = useMemo(() => {
    const sorted = [...data].sort((a, b) => {
      const aValue =
        metric === 'cost' ? a.cost : metric === 'tokens' ? a.tokens : a.value;
      const bValue =
        metric === 'cost' ? b.cost : metric === 'tokens' ? b.tokens : b.value;
            ...
            {chartData.map((model) => (
              <div key={model.id}>
                <div
                  ...
                  onClick={
                    model.isOther ? () => setShowOthers(!showOthers) : undefined
                  }
                >
