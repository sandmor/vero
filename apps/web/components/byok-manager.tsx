...
export function ByokManager() {
    ...
    const [customProviderForm, setCustomProviderForm] = useState<CustomProviderFormData>(defaultCustomProviderForm);
    ...
    const { data: customProvidersData, isLoading: isLoadingCustomProviders } = useQuery({
    ...
    const setFeedbackState = useCallback((id: string, state: FeedbackState, ttl = 1600) => {
    ...
    const saveKeyMutation = useMutation({
        ...
        onSuccess: (_, variables) => {
            toast({ type: 'success', description: `${displayProviderName(variables.providerId)} saved` });
            setFeedbackState(variables.providerId, 'saved');
            queryClient.invalidateQueries({ queryKey: ['byok-provider-keys'] });
        },
        onError: (error, variables) => {
            toast({ type: 'error', description: error instanceof Error ? error.message : 'Failed to save' });
    ...
    const deleteKeyMutation = useMutation({
        mutationFn: async ({ providerId }: { providerId: string }) => {
            const res = await fetch(`/api/user/byok/provider-keys?providerId=${encodeURIComponent(providerId)}`, {
                method: 'DELETE',
            });
            if (!res.ok) {
                const text = await res.text();
        ...
        onSuccess: (_, variables) => {
            ...
            toast({ type: 'success', description: `${displayProviderName(variables.providerId)} removed` });
            setFeedbackState(variables.providerId, 'deleted');
            queryClient.invalidateQueries({ queryKey: ['byok-provider-keys'] });
            queryClient.invalidateQueries({ queryKey: ['byok-models'] });
        },
        onError: (error, variables) => {
            toast({ type: 'error', description: error instanceof Error ? error.message : 'Failed to delete' });
            setFeedbackState(variables.providerId, 'error', 2200);
        },
    });
    ...
    const createCustomProviderMutation = useMutation({
        ...
        onError: (error) => {
            toast({ type: 'error', description: error instanceof Error ? error.message : 'Failed to create' });
        },
    });

    const updateCustomProviderMutation = useMutation({
        mutationFn: async ({ id, ...data }: { id: string } & Partial<CustomProviderFormData>) => {
            const res = await fetch('/api/user/byok/custom-providers', {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
        ...
        onError: (error) => {
            toast({ type: 'error', description: error instanceof Error ? error.message : 'Failed to update' });
        },
    });

    const deleteCustomProviderMutation = useMutation({
        mutationFn: async (id: string) => {
            const res = await fetch(`/api/user/byok/custom-providers?id=${encodeURIComponent(id)}`, {
                method: 'DELETE',
            });
            if (!res.ok) {
                const text = await res.text();
    ...
    const createModelMutation = useMutation({
        mutationFn: async (data: ModelFormData) => {
            const payload = {
                ...
                providerId: data.sourceType === 'platform' ? data.providerId : undefined,
                customProviderId: data.sourceType === 'custom' ? data.customProviderId : undefined,
    ...
    const updateModelMutation = useMutation({
        mutationFn: async ({ id, ...data }: { id: string } & Partial<ModelFormData>) => {
            const payload = {
                id,
                displayName: data.displayName,
                supportsTools: data.supportsTools,
    ...
    const deleteModelMutation = useMutation({
        mutationFn: async (id: string) => {
            const res = await fetch(`/api/user/byok/models?id=${encodeURIComponent(id)}`, {
                method: 'DELETE',
            });
            if (!res.ok) {
                const text = await res.text();
    ...
    const isLoading = isLoadingKeys || isLoadingCustomProviders || isLoadingModels;
    ...
    const canAddModel = providersWithKeys.length > 0 || (customProvidersData?.providers.length ?? 0) > 0;
    ...
    return (
        <div className="space-y-8">
            ...
            <Card className="border-border/40 bg-card/50 backdrop-blur shadow-sm">
                ...
                <CardContent className="space-y-3">
                    {byokProviders.map((provider, index) => {
                        ...
                        const isSaving = saveKeyMutation.isPending && saveKeyMutation.variables?.providerId === p;
                        const isDeleting = deleteKeyMutation.isPending && deleteKeyMutation.variables?.providerId === p;
                        ...
                        return (
                            ...
                            >
                                <Collapsible
                                    ...
                                    onOpenChange={(o) => setProviderOpen((s) => ({ ...s, [p]: o }))}
                                >
                                                ...
                                                >
                                                    <div className="flex flex-col gap-3 md:flex-row md:items-center">
                                                        <Input
                                                            ...
                                                            onChange={(e) =>
                                                                setProviderKeys((k) => ({ ...k, [p]: e.target.value }))
                                                            }
                                                            className="md:flex-1"
                                                        />
                                                    ...
                                                    <p className="mt-2 text-xs text-muted-foreground">
                                                        Your {displayProviderName(p)} API key is stored securely. After saving,
                                                        add models below to use them.
                                                    </p>
                                                    <AnimatePresence initial={false}>
                                                        {status === 'error' && (
                                                            <motion.p
                ...
                <CardHeader>
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <CardDescription>
                                Add OpenAI-compatible endpoints like Ollama, vLLM, or other self-hosted models.
                            </CardDescription>
                        </div>
                        <Button size="sm" className="gap-1.5" onClick={() => openCustomProviderDialog()}>
                                ...
                                >
                                    ...
                                    <div className="flex items-center gap-1">
                                        ...
                                        <Button
                                            ...
                                            onClick={() => deleteCustomProviderMutation.mutateAsync(provider.id)}
                ...
                <CardHeader>
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <CardDescription>
                                Models configured with your API keys. These appear in the model selector.
                            </CardDescription>
                        </div>
                        <Button
                            size="sm"
            ...
            <Dialog open={showCustomProviderDialog} onOpenChange={(open) => {
                ...
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>
                            {editingCustomProvider ? 'Edit Custom Provider' : 'Add Custom Provider'}
                        </DialogTitle>
                        <DialogDescription>
                    ...
                    <div className="space-y-4 py-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <Input
                                ...
                                onChange={(e) =>
                                    setCustomProviderForm((f) => ({ ...f, baseUrl: e.target.value }))
                                }
                            />
                            <p className="text-xs text-muted-foreground">
                        ...
                        <div className="space-y-2">
                            <Label htmlFor="apiKey">
                                API Key {editingCustomProvider && '(leave empty to keep existing)'}
                            </Label>
                            <Input
                                id="apiKey"
                                type="password"
                                placeholder={editingCustomProvider ? '••••••••' : 'Optional API key'}
                                value={customProviderForm.apiKey}
                                onChange={(e) =>
                                    setCustomProviderForm((f) => ({ ...f, apiKey: e.target.value }))
                                }
                            />
                        </div>
                    </div>
                    <DialogFooter>
                        ...
                        >
                            {createCustomProviderMutation.isPending || updateCustomProviderMutation.isPending
                                ? 'Saving…'
                                : editingCustomProvider
                                    ? 'Save Changes'
                                    : 'Create'}
            ...
            <Dialog open={showModelDialog} onOpenChange={(open) => {
                ...
                <DialogContent className="sm:max-w-[500px]">
                    ...
                    <div className="space-y-4 py-4">
                            ...
                            <>
                                <div className="space-y-2">
                                    ...
                                    >
                                        ...
                                        <SelectContent>
                                            <SelectItem value="platform" disabled={providersWithKeys.length === 0}>
                                                Platform Provider
                                            </SelectItem>
                                            <SelectItem
                                                value="custom"
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Input
                                        ...
                                        onChange={(e) =>
                                            setModelForm((f) => ({ ...f, providerModelId: e.target.value }))
                                        }
                                    />
                                    <p className="text-xs text-muted-foreground">
                        ...
                        <div className="space-y-2">
                            <Label htmlFor="maxOutputTokens">Maximum Output Tokens (optional)</Label>
                            <Input
                                id="maxOutputTokens"
                                type="number"
                                ...
                                onChange={(e) =>
                                    setModelForm((f) => ({ ...f, maxOutputTokens: e.target.value }))
                                }
                            />
                            <p className="text-xs text-muted-foreground">
                        ...
                        <Button
                            ...
                            disabled={
                                ...
                                (!editingModel && (
                                    ...
                                    (modelForm.sourceType === 'platform' && !modelForm.providerId) ||
                                    (modelForm.sourceType === 'custom' && !modelForm.customProviderId)
                                ))
                            }
                        >
