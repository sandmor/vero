...
export async function createUserCustomProvider(
    ...
    // Validate slug
    if (!/^[a-z0-9][a-z0-9-]{0,62}[a-z0-9]?$/.test(data.slug) || data.slug.includes('--')) {
        throw new Error('Invalid provider slug. Use lowercase letters, numbers, and hyphens.');
    }
...
function toUserByokModelRecord(
    r: {
        id: string;
        userId: string;
        sourceType: string;
    ...
    if (sourceType === 'platform' && r.providerId) {
        fullModelId = formatByokPlatformModelId(r.providerId as any, r.providerModelId);
    } else if (sourceType === 'custom' && r.customProvider?.slug) {
        fullModelId = formatByokCustomModelId(r.customProvider.slug, r.providerModelId);
    } else {
...
export async function updateUserByokModel(
    ...
    const record = await prisma.userByokModel.update({
        ...
        data: {
            ...
            ...(data.supportsTools !== undefined && { supportsTools: data.supportsTools }),
 ...
 */
export async function getUserByokConfig(userId: string): Promise<UserByokConfig> {
