...
export class TabLeaderElection {
    ...
    async start(): Promise<void> {
        ...
        if (existingLease && existingLease.expiresAt > now) {
            ...
            if (existingLease.tabId === this.tabId) {
                ...
                this.log('Another tab holds the lease:', existingLease.tabId.slice(0, 8));
                this.state = 'follower';
    ...
    private tryAcquireLease(): boolean {
        ...
        if (currentLease && currentLease.tabId !== this.tabId && currentLease.expiresAt > now) {
            return false;
        }
    ...
    private handleMessage = (event: MessageEvent<BroadcastMessage>): void => {
        ...
        this.log('Received message:', message.type, 'from:', message.tabId.slice(0, 8));
            ...
            case 'election-started':
                ...
                if (this.state === 'leader') {
                    this.broadcast({
                        type: 'leader-heartbeat',
                        tabId: this.tabId,
                        expiresAt: this.readLease()?.expiresAt ?? Date.now() + LEADER_LEASE_DURATION_MS,
                    });
                }
                break;
...
export function initializeTabLeader(options: TabLeaderOptions): TabLeaderElection {
    if (globalTabLeader) {
