...
export function applySettingsToMetadata(
  ...
  if (previousSettings) {
    ...
    if (
      compareDefaults(
        previousSettings.newChatDefaults,
...
export function shouldRefreshSettings(
  ...
  const lastSettingsSync = (metadata as any).settingsSyncedAt as
    | string
    | undefined;
  ...
  async sync(options: {
    ...
    const runSync = async () => {
      try {
        ...
        const { updatedMetadata, hasChanges, changes } =
          applySettingsToMetadata(options.currentMetadata, settings);
