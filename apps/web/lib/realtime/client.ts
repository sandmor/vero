...
export class RealtimeClient {
  ...
  private readonly options: Required<Omit<RealtimeClientOptions, 'debug'>> & {
    debug: boolean;
  };
    ...
    try {
      ...
      switch (message.type) {
        case WsMessageType.SUBSCRIBED:
          this.log(
            'Subscribed',
            (message.payload as SubscribedPayload)?.userId
          );
          break;
  ...
  private scheduleReconnect(): void {
    ...
    this.log(
      `Scheduling reconnect attempt ${this.reconnectAttempt} in ${Math.round(delay)}ms`
    );
