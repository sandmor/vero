...
const chatMeta = new Map<string, { docId: string; lastUpdatedAt: string; messageDocIds: string[] }>();
...
function buildSnippet(
    ...
    const searchTerms = [
        ...phrases,
        ...terms.filter((t) => !phrases.some((p) => p.toLowerCase().includes(t.toLowerCase()))),
    ];
    ...
    for (const match of allMatches) {
        if (match.end > windowStart && match.start < windowEnd) {
            ...
            const highlightEnd =
                Math.min(windowEnd - windowStart, match.end - windowStart) + prefixOffset;
 ...
 */
async function loadIndexFromStorage(): Promise<{ loaded: boolean; chatCount: number }> {
...
async function handleLoad(): Promise<{ fromStorage: boolean; chatCount: number }> {
    if (!initialized) {
...
function removeChat(chatId: string): { removed: boolean; messageDocIds: string[] } {
...
async function handleSearch(
    ...
    const allMessageDocIds = parsedQuery.hasComplexBooleans
        ? new Set(
            Array.from(chatMeta.values()).flatMap((m) => m.messageDocIds)
        )
        : undefined;
    ...
    const sortedChatResults = sortChatResults(filteredChatResults, options).map((r) => ({
        chatId: r.chatId,
        score: r.score,
    }));

    const sortedMessageResults = sortMessageResults(filteredMessageResults, options);
    ...
    const run = async () => {
        try {
            ...
            if (payload.type === 'search') {
                const result = await handleSearch(payload.query, payload.options, payload.knownChatIds);
                respond({
                    type: 'searchResults',
                    requestId,
