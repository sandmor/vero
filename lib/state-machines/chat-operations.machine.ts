...
import { setup, assign, fromPromise, type ActorRefFrom, type DoneActorEvent } from 'xstate';
...
export const chatOperationsMachine = setup({
  ...
  actions: {
    ...
    applyFetchedTree: assign(({ context, event }) => {
      ...
      const tree = (event as unknown as DoneActorEvent<MessageTreeResult>).output;
      if (!tree) return {};
  ...
  states: {
    ...
    branchSwitch: {
      ...
      states: {
        ...
        rollingBack: {
          entry: ['rollback', 'restoreRollbackState', 'showBranchSwitchErrorToast'],
