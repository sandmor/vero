...
export function search<T extends Record<string, unknown>>(
    ...
    if (allTerms.length === 0 && Object.keys(parsedQuery.fieldQueries).length === 0) {
        return [];
    }
    ...
    for (const item of items) {
        ...
        for (const [field, queryValue] of Object.entries(parsedQuery.fieldQueries)) {
        ...
        if (allMatches.length > 0 || Object.keys(parsedQuery.fieldQueries).length > 0) {
            const minScore = options.minScore ?? 0;
            const normalizedScore = totalScore / Math.max(allTerms.length, 1);

            if (normalizedScore >= minScore || Object.keys(parsedQuery.fieldQueries).length > 0) {
                results.push({
                    item,
